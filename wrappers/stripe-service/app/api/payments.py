...
async def create_session(body: CreateSessionRequest):
    amount = body.amount_cents if body.amount_cents is not None else settings.CONSULTATION_FEE_CENTS
    currency_value = body.currency if body.currency is not None else settings.CURRENCY
...
def _extract_amount_cents(obj: dict) -> Optional[int]:
    if obj.get("amount_total") is not None:
        return obj.get("amount_total")
