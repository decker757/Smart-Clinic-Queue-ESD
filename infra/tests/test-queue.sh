...
req -X POST "$BASE_QUEUE/queue/reset"
...
req "$BASE_QUEUE/queue/position/$APPT1_ID"

echo ""
echo "=== 8. Get queue position for appointment 2 (expect queue_number=2) ==="
req "$BASE_QUEUE/queue/position/$APPT2_ID"

echo ""
echo "=== 9. Get queue position for non-existent appointment — expect 404 ==="
req "$BASE_QUEUE/queue/position/00000000-0000-0000-0000-000000000000"

# ─── Check-in ────────────────────────────────────────────────

echo ""
echo "=== 10. Check in appointment 1 (waiting → checked_in) ==="
req -X POST "$BASE_QUEUE/queue/checkin/$APPT1_ID"
...
req -X POST "$BASE_QUEUE/queue/no-show/$APPT3_ID"
